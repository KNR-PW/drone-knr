...
def data():
    db = get_db()
    cur = db.cursor()
    rs = cur.execute('SELECT d.id AS detection_id, d.date, d.lat, d.lon, d.image_name, t.id AS tree_id, t.color FROM detection d LEFT JOIN tree t ON t.detection = d.id GROUP BY tree_id').fetchall()
    
    # i don't know about the idiomatic way to do this...
