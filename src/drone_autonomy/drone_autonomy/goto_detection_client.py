...
class GotoDetectionClient(Node):
    ...
    def send_request(self, info=0, gps=[0.0,0.0,0.0], yaw=0.0):
        ...
        return future.result().detections_list.detections_list[0].gps_position
...
def main(args=None):
    ...
    goto_detection_client = GotoDetectionClient()
    ...
    #     time.sleep(1)
    for i in range(1):
        gps = goto_detection_client.get_gps()
        yaw = goto_detection_client.get_yaw()
        gps_position = goto_detection_client.send_request(gps=gps, yaw=yaw)
        goto_detection_client.get_logger().info(f"Going to: x:{gps_position[0]}     y:{gps_position[1]}")
        goto_detection_client.send_goto_relative(gps_position[0], gps_position[1], 0.0)
    goto_detection_client.destroy_node()
